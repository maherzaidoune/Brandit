  ...
  export default (state = INITIAL_STATE, action) => {
    ...
    switch (action.type) {
      ...
      case GET_MASK_SUCCESS:
        return {...state, isRequesting: false, masq: action.payload.data.filter(i => i.orientation == 2).map(v => state.imageUrl + v.image) , landmasq: action.payload.data.filter(i => i.orientation == 2).map(v => state.imageUrl + v.image)};
